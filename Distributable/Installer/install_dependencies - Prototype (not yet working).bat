...
set "CUDA_VERSION=12.1"  rem Update to CUDA version 12.1
set "CUDA_URL=https://developer.download.nvidia.com/compute/cuda/%CUDA_VERSION%/local_installers/cuda_%CUDA_VERSION%_win.exe"
set "CUDA_INSTALLER=%INSTALL_DIR%\cuda_%CUDA_VERSION%_win.exe"
...
rem Function to check and install CUDA if needed
:install_cuda
rem Check if CUDA is already installed
nvcc --version > nul 2>&1
if errorlevel 1 (
    echo CUDA is not installed. Installing CUDA %CUDA_VERSION%...
    bitsadmin /transfer cudaInstaller /download /priority normal "%CUDA_URL%" "%CUDA_INSTALLER%"
    if errorlevel 1 (
        echo Failed to download CUDA installer. Please check your internet connection or download manually from:
        echo %CUDA_URL%
        pause
        exit /b 1
    )
    ...
    rem Check if CUDA installation was successful
    nvcc --version > nul 2>&1
    if errorlevel 1 (
        echo CUDA installation should be finished. Close the installer and run it again. If the problem persists, install it manually. Press any key to close.
        pause
        exit /b 1
    ) else (
