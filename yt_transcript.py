...
def get_video_lang(logger, args, link, filename_without_extension):
    ...
    sample_audio_path = os.path.join(args.audiopath, f'{filename_without_extension}.mp3')
...
def summary_video_from_link(clean_vtt, logger, args, link, get_video_lang):
    ...
    filename = subprocess.check_output(get_dl_audio_path_cmd, shell=True).decode('utf-8').strip()
    ...
    if os.path.exists(subtitle_file):
        os.rename(subtitle_file,f"{args.text_output_dir}{filename_without_extension}.vtt")
    ...
    def chunk_string_by_words(string, length):
        ...
        space_differences = [j - i for i, j in zip(space_positions[:-1], space_positions[1:])]
        uniformity = max(space_differences) - min(space_differences) if space_differences else 0
        ...
        if space_percentage > 0.05 and uniformity <= length:
            words = string.split()
            chunks = [' '.join(words[i:i+length]) for i in range(0, len(words), length)]
        else:
    ...
    post_text_dir = os.path.join(args.post_text_output_dir, f'{filename_without_extension}.txt')
    ...
    if (detected_language == 'en') & (language == 'zh'):
        language = 'en'
    if ((detected_language == 'zh-cn')|(detected_language == 'zh-cn')|(detected_language == 'zh'))  &( language == 'en'):
        language = 'zh'
    ...
    # Run TTS
    post_audio_dir=f"{os.path.join(args.post_audio_output_dir, filename_without_extension)}.wav"
    # Split the response_text into chunks of 350 characters
    chunks = chunk_string_by_words(response_text, 200)

    # Generate the audio files
    for i, chunk in enumerate(tqdm(chunks)):
        tts.tts_to_file(chunk, file_path=f"{post_audio_dir}_temp_{i}.wav", speaker='Tammie Ema', language=f"{language}")
...
def integrate_text_format(video_title,args,vtt_file,llm_summary):
    ...
    # 將兩個文件的內容合併
    integrate_text = video_title + '\n(' + link+ ')\n' + '#'*16 + '\n' + 'prompt:' + args.prompt + '\n' + llm_summary + '\n' + '#'*16 + '\n' + vtt_file
    

    # 將合併後的內容寫入一個新的文件
    with open(f'{args.integrate_text_output_dir}{video_title}.txt', 'w',encoding='utf-8') as file:
...
file_handler = RotatingFileHandler(f'{script_dir}/logfile.log',maxBytes=10**6,backupCount=2)
...
parser = argparse.ArgumentParser(description='Download youtube video, convert it to text and inference by llm')
## yt download parameters
parser.add_argument('--link', type=str,default='', help='The link to the youtube video,or a ttrss link')
parser.add_argument('--prompt', type=str, default="summary the following content", help='template to llm, in en or zh')
parser.add_argument('--language', type=str, default= "en", help='language of output')
parser.add_argument('--whisper_model_size', type=str, default='medium', help='model to use for whisper')
parser.add_argument('--model_name', type=str, default="auto", help='llm model,in ollama format:https://ollama.com/library')
parser.add_argument('--timestamp_content', type=str, default="False", help='use timestamp in LLM inference')
parser.add_argument('--output_dir', type=str,default= script_dir, help='output directory')
parser.add_argument('--audiopath', type=str, default='./audio/', help='The path to the audio file to save to')
parser.add_argument('--text_output_dir', type=str, default='./text/', help='The path to the text file to save to')
parser.add_argument('--integrate_text_output_dir', default="./integrate_text/", type=str, help='integrate_text_output_dir')
parser.add_argument('--post_text_output_dir', default="./text_llm_processed/", type=str, help='post_text_output_dir')
parser.add_argument('--post_audio_output_dir', default="./audio_llm_processed/", type=str, help='post_audio_output_dir')
